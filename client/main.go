...
func main() {
	...
	ticketConn, _ := grpc.Dial("localhost:50052", grpc.WithInsecure())
	...
	loginResp, err := userClient.Login(ctx, &userpb.LoginRequest{
		Username: username,
		Password: password,
	})
	...
	if err != nil || !loginResp.Success {
		...
		registerResp, err := userClient.Register(ctx, &userpb.RegisterRequest{
			Username: username,
			Password: password,
			Role:     roleInput,
		})
		...
		if err != nil || !loginResp.Success {
			log.Fatalf("Errore di login dopo la registrazione: %v", err)
		}
	}
	...
	for {
		...
		switch choice {
		case "1":
			listResp, err := ticketClient.ListEvents(ctx, &ticketpb.ListEventsRequest{})
			if err != nil {
		...
		case "2":
			...
			resp, _ := ticketClient.PurchaseTicket(ctx, &ticketpb.PurchaseTicketRequest{
				EventId:  eventID,
				Quantity: qty,
				UserId:   userID,
			})
		...
		case "3":
			...
			resp, _ := ticketClient.AddEvent(ctx, &ticketpb.AddEventRequest{
				UserId:           userID,
				Name:             name,
				Date:             date,
		...
		case "4":
			...
			resp, _ := ticketClient.DeleteEvent(ctx, &ticketpb.DeleteEventRequest{
				UserId:  userID,
				EventId: eid,
			})
			fmt.Println(resp.Message)
...
func main() {
	...
	if err != nil || !loginResp.Success {
		...
		registerResp, err := userClient.Register(registerCtx, &userpb.RegisterRequest{
			Username: username,
			Password: password,
			Role:     roleInput,
		})
		...
		loginResp, err = userClient.Login(loginCtx, &userpb.LoginRequest{
			Username: username,
			Password: password,
		})
		if err != nil || !loginResp.Success {
	...
	for {
		...
		case "2":
			...
			resp, err := ticketClient.PurchaseTicket(ctx, &ticketpb.PurchaseTicketRequest{
				EventId:  eventID,
				Quantity: qty,
				UserId:   userID,
			})
		...
		case "3":
			...
			fmt.Println("➡️ Chiamata AddEvent in corso...")
			resp, err := ticketClient.AddEvent(ctx, &ticketpb.AddEventRequest{
				UserId:           userID,
				Name:             name,
				Date:             date,
				AvailableTickets: qty,
			})
			fmt.Println("⛔ Chiamata AddEvent completata.")
			if err != nil {
		...
		case "4":
			...
			resp, err := ticketClient.DeleteEvent(ctx, &ticketpb.DeleteEventRequest{
				UserId:  userID,
				EventId: eid,
			})
			if err != nil {
...
// Retry per chiamate gRPC con ritorno di una risposta
